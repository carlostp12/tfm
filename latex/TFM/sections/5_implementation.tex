...
Utilizing a custom Python framework, a representative sample from the 2dFGRS catalog was benchmarked against a synthetic Poisson distribution (random catalog) generated to match the survey's spatial constraints. To ensure the robustness of the results, four distinct estimators were evaluated by computing the normalized pair counts—Data-Data ($DD$), Random-Random ($RR$), and Data-Random ($DR$).
