...
For the present study, we will utilize the coordinate system referred to as the Re-Real space. This space is particularly relevant as it incorporates the effects of both the Kaiser and FoG distortions.
...
Figure \ref{fig:slos} illustrates the elongated Euclidean distance’s effect on the OPTICS clustering results. Clusters are computed in an elongated way along the line of sight in order to cure the redshift space distortion.
	...
	\begin{figure}[h]
	\centering
	\includegraphics[width=0.5\textwidth]{./figs/slos.jpg}
...
\begin{minipage}{.5\textwidth}
  \centering
  \includegraphics[width=.5\linewidth]{./figs/dpc_11.jpg}
  \caption{Different colors correspond to different clusters.}
  \label{fig:group}
\end{minipage}%
\begin{minipage}{.5\textwidth}
  \centering
  \includegraphics[width=.5\linewidth]{./figs/dpc_12.jpg}
...
Then we can select $\rho = \rho_{0}$ and $\delta = _{0}$ to obtains centers for clusters:
	...
	\begin{figure}[h]
	\centering
	\includegraphics[width=0.5\textwidth]{./figs/dpc_3.jpg}
