...
Redshift survey distortions are primarily categorized into two distinct sets:
...
Following the methodology proposed in \cite{Shi:2016}, the authors define several coordinate spaces by utilizing Equations \ref{eq:6}, \ref{eq:7}, and \ref{eq:4} to recalculate the positions (Equation \ref{eq:2}) in each space.

For the present study, we will utilize the coordinate system referred to as the Real space which when reconstructted based on correcting redshift space distortions is called Re-Real space \cite{Shi:2016}. This space is particularly relevant as it incorporates the effects of both the Kaiser and FoG distortions.

Employing this approach yields a modified catalog where the new redshift values effectively transform the coordinate space, as illustrated in Figure \ref{fig:real} . We will use this Real space dataset to thoroughly investigate how this transformation influences the resulting clustering of galaxies.
...
Then we can select $\rho = \rho_{0}$ and $\delta = _{0}$ to obtains centers for clusters:
	...
	\caption{ $\rho = \rho_{0}$ and $\delta = \delta_{0}$ inside decision graph }{Source \cite{DPC:2014}.}
	\label{fig:mst}
	\end{figure}
