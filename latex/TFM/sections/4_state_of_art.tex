...
\section{Redshift to distance}

The redshift is a distance indicator, at firts the Hubble Rule was used:
\begin{equation} \label{eq:1}
	D=HV
\end{equation}	
where H is the Hubble constant. It was easy to see \ref{eq:1} works only as approximation for nearby galaxies, instead a cosmologicval model need to be apply. The most newest models says that:

\begin{equation} \label{eq:2}
	D_p = \frac{1}{H_0} \int_0^z \frac{dz}{ \sqrt{\sum_i \Omega_{i0} (1+z)^{3(1+w^i)}}} =  \frac{1}{H_0} \int_0^z \frac{dz}{ \sqrt{\sum_i \Omega_{r0} (1+z)^{4} + \Omega_{m0} (1+z)^{3} + \Omega_{\Lambda 0}  } }
\end{equation}.
Now, depending on the model of universe we take equation \ref{eq:1} may vary, for this study we choose the values of the so called \Lambda CDM or concordant-model, we assume:
...
\begin{itemize}
		...
		\item The poinit p is density connected to a point q with respect to Eps and MinPts if there is thirdth point o such that both p and q are density-reachable from o with respect to Eps and MinPts.
\end{itemize}		
Then a cluster C is a subset of S satisfying:
	\begin{itemize}
		...
		\item \(\forall p, q, \in C\) if q is density reachable from p with respect to Eps and MinPts then \(q \in C \). This property is called sometimes as Maximallity.
	\end{itemize}

Then with this method, DBSCAN creates a set of clusters \(C_1, ..., C_k\). All points in S are classified as
\begin{enumerate}
		\item Core-points: points with a dense neighborhood.
		\item Border-points: points belonging to a cluster but without a dense neighborhood.
		\item Noise points: points do not belonging to any cluster.
\end{enumerate}
...
We will see it in next pseupdo-code:
