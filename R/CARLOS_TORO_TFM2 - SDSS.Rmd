...
dt <- read.csv('./groups/sdss/SDSS-dist.csv')
str(dt)
```
...
#dt_sample3 <- dt_sample3[dt$RA<210 & dt$RA>180 & dt$DEC>25 & dt$DEC<30,]

dt_sample3<- dt_sample3[,c('GAL_ID','x', 'y', 'z', 'Z',  "dist", "GROUP_ID")]
dt_sample3 <- dt_sample3[dt_sample3$Z< max_redshift,]
#Change column name
...
```{r}
blo_scan <- extractDBSCAN(res, eps_cl = 0.00055)
...
undetected_groups <- function(dataset){
  groups_results <-sqldf("select distinct(GROUP_ID) FROM h2
    where group_id not in (SELECT distinct(group_id) AS group_id
		FROM dataset)")
   groups_results
} 
```
...
# Plot a couple of vector in fucntion of a sequence
plot_purity_completeness <- function(eps_sequence, purity_l, completeness_l, pures_com,  titles = NULL ,title = ""){

if (is.null(pures_com)) {
	df <- data.frame('eps' = eps_sequence, 'Purity'= purity_l, title2= completeness_l)
	if(length(titles)>0){
	  setNames(df, titles)
	}
	head(df)
	df %>%
    gather(Values,perc, Purity, Completeness) %>%
    ggplot(aes(x=eps , y=perc, colour=Values)) +
    geom_line()+
    ggtitle(title)
 }else{
	df <- data.frame('eps' = eps_sequence, title1= purity_l, title2= completeness_l, title3= pures_com)
	if(length(titles)>0){
	  setNames(df,titles)
	}
	head(df)
	df %>%
    gather(Values,perc, Purity, Completeness, pures_com) %>%
    ggplot(aes(x=eps , y=perc, colour=Values)) +
    geom_line()+
    ggtitle(title)
...
# Pretty print a vector pecceed by a title
...
eps_sequence_test <- seq(0.0001, 0.0004, 0.000025)
...
plot_purity_completeness_trend(
  ...
  "Purity/completness on not scaled data",
  3.8e-4
)
```

Because of this char we know that optimal point is around $0.00055$
...
plot_purity_completeness(
  ...
  x_stats$completes/true_groups, 
  x_stats$pure_complet/true_groups,
  c('Purity', 'Completeness', 'Pur.+Com.'),
  "Group global group purity and completness on not scaled data",
  3.8e-4
)
```
...
blo_scans <- extractDBSCAN(ress, eps_cl = 0.012)
```

Again we can do the same for scaled data:

```{r}
 eps_sequence_test <- seq(0.01, 0.04, 0.005)
...
plot_purity_completeness(
  eps_sequence_test, 
  x_stats$group_list,
  x_stats$cluster_list,
  ones(length(eps_sequence_test),1)*true_groups,
   c('Purity', 'Completeness', 'True groups.'),
  "Complete and pure groups out of true groups",
  0.018
)
```
...
eps_sequence_test <- seq(0.00005, 0.0004, 0.00005)
