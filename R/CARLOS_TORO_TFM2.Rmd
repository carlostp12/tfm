...
print(sprintf("Number of groups with more than %s members: %s", min_members, true_groups))
```
...
barplot(ttable, col=('red'), 
        main=sprintf("Groups (%s) with at least %s members ", true_groups, min_members))
```
...
For this study we consider an output-cluster to be pure if P\>=0.66 (at least 2/3 of elements of an output-cluster belong to a single group).
An original cluster is complete if C\>=0.5 (at least half data belong to an original true group)
...
galaxyClusters <- findClusters(galaxyDens, rho=0.99, delta=0.0012)
plot(galaxyClusters)
