...
get_elements_not_in <- function(dataset_mm, dataset_all){
...
get_elements_in <- function(dataset_mm, dataset_all){
  groups_results <- sqldf("select 
	  	  GAL_ID,
	      x, 
	      y, 
...
print(
  sprintf('Number of galaxies in groups with more than %s elements %s out of %s, aprox %s ', 
          min_members, 
...
**Purity(P)**: measure of output-cluster: proportion of members coming exclusively from a single true group, providing confidence that the algorithm correctly groups members together. A high purity rate indicates the algorithm's effectiveness in identifying true groupings.
...
print_global_stats <- function(global_stats, sequence_values) {
  pretty_print(pretty_print("Completeness", global_stats$completeness_list))
  pretty_print(pretty_print("Purity", global_stats$purity_list))
  pretty_print(pretty_print("Groups", global_stats$group_list))
  pretty_print(pretty_print("Clusters", global_stats$cluster_list))
  pretty_print(pretty_print("EPS", sequence_values))
  pretty_print(paste("True Groups", true_groups))
  pretty_print(pretty_print("Und. Groups", global_stats$und_gr))
  pretty_print(pretty_print("Complete gr.:", global_stats$completes))
  pretty_print(pretty_print("Pure gr.:", global_stats$pures))
  pretty_print(pretty_print("Fr:", global_stats$fr_list))
  pretty_print(pretty_print("Fp:", global_stats$fp_list))
  pretty_print(pretty_print("Spurious:", global_stats$spurious))
  pretty_print(pretty_print("Bad class:", global_stats$bad_class))
  pretty_print(pretty_print("Recovery:", global_stats$recovery))
}
```

Following is intended to calculate stats
...
```{r}
plottable <- list('uno' = x_stats$purity_list,
                  'uno' = x_stats$completeness_list,
                  'uno' =  x_stats$fr_list,
                  'uno' =  x_stats$fp_list
                  )
titles <-  c('Purity', 'Completeness', "FR" , "FPplo")
atitle  <-  "Purity/completeness on not scaled data"
xlab <- 'eps'
ylab <- 'percentage'
x <- 0.000525
pplot_purity_completeness(
  eps_sequence_test, 
  plottable, 
  titles, 
  title=atitle, 
...
pretty_print(pretty_print("Recovery:", x_stats$recovery))
...
plot_purity_completeness(
    ...
    x_stats$completeness_list,
    x_stats$recovery, 
    c('Purity', 'Undetected groups', 'Recovery'),
    "Group purity and undetected stats on not scaled data",
    0.0230, 'eps', 'Percentage'
)
...
get_elements_not_in_groups <- function(mm5, dataset_all){
  groups_results <- sqldf("select 
	  ...
	  where 
		...
		not in (select 
		  			group_id 
			    from dataset_all) group by mm5.GAL_ID,
	      mm5.x, 
	      mm5.y, 
	      mm5.z, 
	      mm5.GROUP_ID, 
...
```{r}
eps <- 0.025
blo_scans <- extractDBSCAN(ress, eps_cl = eps)
mm$cluster_id <- blo_scans$cluster

all <- execute_stats(mm, blo_scans)

undetected <- get_elements_not_in_groups(mm5, all)
detected <- get_elements_in_groups(mm5, all)
...
#use this instead
plot3D_cluster(galaxyClusters, points)
...
+-----------+------------+-----------+-------------+
| Method    | DataSample | Outcomes  | Conclussion |
+===========+============+===========+=============+
