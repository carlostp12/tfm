...
mm <- dt[ dt$ra<= ra_lim_sup & 
              dt$ra>=ra_lim_inf & dt$dec>= dec_lim_inf & 
              dt$dec<=dec_lim_sup & dt$redshif< max_redshift,]
...
extract_stats <- function (eps_sequence_values, local_res) {
	...
	#data$stats <- stats
	#data$undetected_groups<-alli[10]
	#(data$stats)
	stats
}
```
...
```{r}
names <- c("purity_list", "completeness_list")
x_stats$eps <- eps_sequence_test
ppplot_purity_completeness(x_stats, names,  "Purity and completeness", 0.000525, "eps", "perc")
```
...
plot3d(points$x, points$y, points$z, 
        col = cl$cluster + 1, size = 2, 
        xlab = "X", ylab = "Y", zlab = "Z")
#points$cluster <- as.factor(cl$cluster)

#points$cluster[points$cluster == "0"] <- "Outlier"

#plot_ly(points, x = ~x, y = ~y, z=~z, color = ~cluster, type = "scatter3d", mode = "markers")
```
...
length(unique(undetected$GROUP_ID))
```
...
lower_tri_values <- f(x[lower_tri_indices[,1]], x[lower_tri_indices[,1]])
