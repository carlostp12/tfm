...
We want, in this work apply some clustering algorithms, and the compare the obtained models with the halo-based group finder optimized for groupping galaxies that reside in the same dark matter halo.

In other words: this is not a normal clustering problem where we want to find a model where most of the samples belong to a group in an hypothetical method maximized by the "Elbow method".

We want find some hyper-parameters configuration makes possible to modeling the data-groups given by the halo-based finder.

We have to clarify that we reserved the term **group** for original and actual clusters in the dataset and **cluster** or **output-cluster** to design the clusters obtained as a result to apply our clustering methods.
...
$$RA \in [90, 1500] h,\, DEC \in [15, 25] º,\, \,and\,\, z \lt max\_redshift$$
...
As we can see, in $\delta=0.00080$ and $\rho=0.9985$ produce the best results with a 99% of group-center detection.
...
+-------------------------------+-------------+-----------------------------+--------------------------------------------------------------------------+
| DPC                           | Non-scaled  | 99% group-center detection. | Good in group center detection, but not in clustering the whole dataset. |
|                               |             |                             |                                                                          |
|                               |             |                             | $\delta=0.0008$ and $\rho=0.9986$                                        |
+-------------------------------+-------------+-----------------------------+--------------------------------------------------------------------------+
| **sOPTICS**                   | **Scaled**  | **P: 0.72**                 | **0.0001**                                                               |
|                               |             |                             |                                                                          |
|                               |             | **C: 0.81**                 |                                                                          |
|                               |             |                             |                                                                          |
|                               |             | **R: 0.69**                 |                                                                          |
...
+-------------------------------+-------------+-----------------------------+--------------------------------------------------------------------------+
| DBSCAN                        | Scaled      | P: 0.69                     | Good in cluster detection with                                           |
|                               |             |                             |                                                                          |
|                               |             | C: 0.69                     | eps = 0.035                                                              |
|                               |             |                             |                                                                          |
|                               |             | R: 0.57                     |                                                                          |
...
In conclusion. In this study we take a sample from the SDSS catalog and selected 7229 objects within an area between 3 and 5h in RA and 15 and 25º in DEC. We tried several methods in groups detecting selecting groups with a minimum of 5 elements (galaxies), we got 95 groups.
...
-   The DPC algorithm worked almost perfectly in group-center detection (reached a 99%), but did not worked in separate the groups in the whole dataset.
...
|--------------------------------|--------------------------------------|
| ![](images/paste-C0421FDF.png) | ![](images/paste-9187A18F.png)       |
