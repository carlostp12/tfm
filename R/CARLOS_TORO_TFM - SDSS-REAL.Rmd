...
we create a heat-map for $\rho$ and $\delta$ hyper-parameters:
...
for (rho in rhos){
	for (delta in deltas){
        ...
        l <- length(unique(peaks[peaks$GROUP_ID>0, c('GROUP_ID')]))
...
As we can see , all groups-center are detected with DPC for values $\delta=0.00085$ and $\rho=0.9986$.

Nevertheless we can check how the points
