...
DPG is not good in create a model of clustering, but it can be good detecting the peaks for each group, for exmaple:
...
```{r}
rhos <- seq(0.9955, 0.9985, 0.0005)
deltas <- seq(0.0005, 0.0009, 0.00005)#seq(0.00070, 0.00087, 0.00001)

j <- 1
i <- length(deltas)
matrix_data=matrix(nrow = length(deltas), ncol=length(rhos))

for (rho in rhos){
  #print(sprintf("Columna %s %s", j, rho))
	for (delta in deltas){
	    #print(sprintf(" Fila %s %s", i, delta))
        value <- assessDPC(delta, rho, mm)
        matrix_data[i, j] <- value
        i <- i-1
       # print(sprintf("   L = %s", min(l/true_groups, true_groups/l)))
    }
    j <- j+1
    i <- length(deltas)
}

#custom_heatmap(matrix_data, rhos, deltas, xTitle = "delta", yTitle = "rho", numColors = 7)
               
custom_heatmap(matrix_data, deltas, rhos, xTitle = "rho", yTitle = "delta", numColors = 10)
```

we create a heat-map for $\rho$ and $\delta$ hyper-parameters, va por lo anterior:

```{r}
#rhos <- seq(0.950, 0.960, 0.005)
#deltas <- seq(0.0001, 0.0005, 0.0001)



#rhos <- seq(0.9950, 0.9954, 0.0001)
#deltas <- seq(0.00095, 0.00105, 0.00001)

#rhos <- seq(0.9956, 0.9962, 0.0001)
#deltas <- seq(0.00088, 0.00098, 0.00001)

#rhos <- seq(0.9966, 0.9978, 0.0001)
#deltas <- seq(0.00086, 0.00099, 0.00001)


#rhos <- seq(0.9955, 0.9968, 0.0001)
#deltas <- seq(0.00094, 0.00110, 0.00001)

rhos <- seq(0.9955, 0.9963, 0.0001)
deltas <- seq(0.00005, 0.0004, 0.00005)#seq(0.00070, 0.00087, 0.00001)
...
for (rho in rhos){
  #print(sprintf("Columna %s %s", j, rho))
	for (delta in deltas){
        ...
        galaxyClusters <- findClusters(galaxyDens, rho=rho, delta=delta)
        peaks <- mm5[galaxyClusters$peaks,]
        l <- length(unique(peaks[peaks$GROUP_ID>0 &
                    !is.na(peaks$GROUP_ID), c('GROUP_ID')]))
...
As we can see , all groups-center are detected with DPC for values $\delta=0.000778$ and $\rho=80.9979$. We can obtain them as follows

```{r}
rho=0.998
delta=0.00077
galaxyClusters <- findClusters(galaxyDens, rho=rho, delta=delta)
peaks5 <- mm5[galaxyClusters$peaks,]
l <- length(peaks5[peaks5$GROUP_ID>0 &  !is.na(peaks5$GROUP_ID), c('GROUP_ID')])
min(l/true_groups, true_groups/l)

peaks <- na.omit(peaks5)

plot3d(peaks$x, peaks$y, peaks$z, col = peaks$GROUP_ID+1, size = 3, xlab = "X", ylab = "Y", zlab = "Z")

```

![](images/clipboard-2231986640.png){width="581"}

Each point in this plot represents a different represent for each cluster
...
## Elbow method for hyperparameter tunning
...
How reader can see, the "elbow mehod" does not apply here pretty well, this is because we are not trying to optimize the out-put clusters given by the algorithm, instead we are trying to fit some density methods in order to detect groups of galaxies generated by black matter halos and grouped by density .
...
+=============================================+=====================================+
| ![](images/paste-BC9FEB7D.png)              | ![](images/paste-D7228FBB.png)      |
+---------------------------------------------+-------------------------------------+
