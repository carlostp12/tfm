...
real_matrix_data=matrix(ncol=length(rhos), nrow = length(deltas)) 
for (rho in rhos){
    for (delta in deltas){
        galaxyClusters <- findClusters(galaxyDens, rho=rho, delta=delta)
        peaks <- mm[galaxyClusters$peaks,]
        l <- length(unique(peaks$GROUP_ID))
        l2 <- length(unique(mm5[galaxyClusters$peaks, 'GROUP_ID']))
#        print(sprintf("FOR %s,  %s detected %s groups for clusters %s ", delta, rho, l2, l))
        matrix_data[i, j] <- min(l/true_groups, true_groups/l)
        real_matrix_data[i, j] <- min(l2/true_groups, true_groups/l2)
        i <- i-1
    }
    j <- j+1
    i<-ll
...
But for these peaks only a 21% of original halo-group are depicted, as shwon at:

```{r}
custom_heatmap(real_matrix_data, deltas, rhos, xTitle = "rho", yTitle = "delta", numColors = 11)
```

As we can see, in $\delta=0.00085$ and $\rho=0.9986$ produce the best results with a 99% of group-center detection, however we can not conclude that all centers represent different original halo-groups, so the center detection did not work.
...
# ![](images/paste-90910F33.png){width="445"}
...
+-------------------------------+-------------+---------------------------------+------------------------------------------------------------------------------------+
| DBSCAN                        | Non-scaled  | P: 0.83 (79%)                   | Optimal point for                                                                  |
|                               |             |                                 |                                                                                    |
|                               |             | C: 0.92 (95%)                   | eps= 0.0003                                                                        |
|                               |             |                                 |                                                                                    |
|                               |             | R: 99 (75%)                     |                                                                                    |
|                               |             |                                 |                                                                                    |
|                               |             | U: 6                            |                                                                                    |
...
+-------------------------------+-------------+---------------------------------+------------------------------------------------------------------------------------+
| DPC                           | Non-scaled  | 100% in groups-center detection | Good in group center detection, but detected centers do not match original groups. |
|                               |             |                                 |                                                                                    |
|                               |             |                                 | $\delta=0.00085$ and $\rho=0.9986$                                                 |
+-------------------------------+-------------+---------------------------------+------------------------------------------------------------------------------------+
| OPTICS                        | Scaled      | \-                              | Good in cluster reachability plot                                                  |
+-------------------------------+-------------+---------------------------------+------------------------------------------------------------------------------------+
| DBSCAN                        | Scaled      | P: 0.88 (87%)                   | eps=0.025                                                                          |
|                               |             |                                 |                                                                                    |
|                               |             | C: 0.90 (96%)                   |                                                                                    |
|                               |             |                                 |                                                                                    |
|                               |             | R: 0.96 (82%)                   |                                                                                    |
...
+=============================================+=====================================+
| ![](images/paste-BC9FEB7D.png)              | ![](images/paste-F0263316.png)      |
+---------------------------------------------+-------------------------------------+
